...
<script>
	export default {
		...
		created: function() {
			// COMPANY //
			this.CEO = this.equityData.company.CEO
			
			// DIVIDENDS // // EARNINGS // // NEWS //
			// QUOTE //
			this.avgTotalVolume = this.formatNumber(this.equityData.quote.avgTotalVolume)
			this.change = this.equityData.quote.change
			this.changePercent = this.equityData.quote.changePercent
			this.marketCap = this.formatNumber(this.equityData.quote.marketCap)
			this.last = this.equityData.quote.latestPrice
			this.peRatio = this.equityData.quote.peRatio
			this.volume = this.formatNumber(this.equityData.quote.volume)
			
			this.sharesOutstanding = this.formatNumber(this.equityData.stats.sharesOutstanding)
			
			// STATS //
			this.day200MovingAvg = this.equityData.stats.day200MovingAvg
			this.day50MovingAvg = this.equityData.stats.day50MovingAvg
			// If there is a dividend..
			if (this.equityData.stats.ttmDividendRate) {
				this.ttmDividendRate = (this.equityData.stats.ttmDividendRate).toFixed(2)
				this.dividendYield = (this.equityData.stats.dividendYield * 100).toFixed(3)
				this.exDividendDate = this.equityData.stats.exDividendDate
				this.paymentDate = this.equityData.dividends[0].paymentDate
			}
			this.nextEarningsDate = this.equityData.stats.nextEarningsDate
			this.float = this.formatNumber(this.equityData.stats.float)
			this.ttmEPS = this.equityData.stats.ttmEPS
			this.week52high = this.equityData.stats.week52high
			this.week52low = this.equityData.stats.week52low
		},
